...
const Sidebar = ({ logoClara }) => {
  ...
  const menuItems = [
    ...
    { id: 'meus-livros', label: 'Meus livros', icon: BookOpen, path: '/meus-livros' },
    { id: 'lista-desejo', label: 'Lista de desejo', icon: Heart, path: '/lista-desejo' },
  ];
  ...
  return (
    <div className="w-64 bg-[#001b4e] h-screen sticky top-0 flex flex-col p-6 text-white shrink-0">
      ...
      <nav className="flex-1 space-y-4">
        {menuItems.map((item) => (
          <Link
            key={item.id}
            to={item.path}
            className={`flex items-center gap-3 w-full p-3 rounded-lg transition-all ${location.pathname === item.path ? 'bg-white text-[#001b4e] font-bold shadow-lg' : 'hover:bg-white/10'
              }`}
          >
      ...
      <div className="mt-auto border-t border-white/20 pt-6 space-y-4">
        ...
        <Link to="/login" className="flex items-center gap-3 w-full p-2 text-red-400 hover:text-red-300 transition-colors text-sm">
          <LogOut size={18} /> Sair
        </Link>
      </div>
    </div>
