  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <button
          onClick={(e) => {
            e.stopPropagation(); // Impede que entre na página do livro ao clicar no coração
            onAddWishlist(livro);
          }}
        ...
        <button
          onClick={(e) => {
            e.stopPropagation(); // Impede que entre na página do livro ao clicar no botão
...
function Catalogo({
  ...
  const navigate = useNavigate(); // <--- Inicializei o hook de navegação
    ...
    <div className="max-w-6xl mx-auto animate-in slide-in-from-bottom-4">
      ...
      <div className="flex gap-8 items-center overflow-x-auto pb-6 scrollbar-hide">
        {destaques.map((livro) => (
          <div
            key={livro.id}
            onClick={() => navigate(`/livro/${livro.id}`)} // <--- CLIQUE PARA NAVEGAR
            className="min-w-[300px] relative group cursor-pointer" // <--- CURSOR POINTER
          >
            <BookCard
              livro={livro}
              size="large"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6 pb-6">
        {recomendacoes.map((livro) => (
          <div
            key={livro.id}
            onClick={() => navigate(`/livro/${livro.id}`)} // <--- CLIQUE PARA NAVEGAR
            className="relative group cursor-pointer" // <--- CURSOR POINTER
          >
            <BookCard livro={livro} showTitle={true} showAuthor={true} />
            {/* Botões de ação flutuantes */}
